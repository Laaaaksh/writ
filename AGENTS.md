...
- `cmd/writ/main.go`'s `version` is declared `const`, so goreleaser's `-X main.version=...` ldflag is a no-op — released binaries will still report `writ version dev` until that identifier becomes a `var`. Flagged, not fixed, since packaging changes must not touch CLI source.
